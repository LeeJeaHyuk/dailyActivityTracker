...
def run_inference():
    base_dir = os.path.dirname(os.path.abspath(__file__))  # 스크립트 파일의 절대 경로
    if platform.system() == "Windows":
        main_path = os.path.join(base_dir, "..", "bin", "llama-cli.exe")
    else:
        main_path = os.path.join(base_dir, "..", "bin", "llama-cli")
    command = [
        f'{main_path}',
        '-m', os.path.join(base_dir, "..", args.model),  # 모델 경로도 절대 경로로
